...
<html lang="en">
	...
	<h1 class="text-center text-danger">Pending Requests <span class="text-dark font-weight-bold">(PARENTS)</span></h1>	
...
<?php
	...
	if(isset($_POST['approve']))
	{
		...
		$query = "UPDATE users SET status='active', role='parent' WHERE id = '$user_id'";
		$query_result = mysqli_query($connection,$query);

		$query2 = "SELECT * FROM parents WHERE id = '$user_id'";
...
$body = "Dear Parent | $fullname | Your Registration request has been approved.
...
PASSWORD: $password";
	...
	if(isset($_POST['reject']))
	{
		...
		if($get_data = mysqli_fetch_array($query2_result))
		{
			...
			$email = $get_data['email'];
			$subject = "Registration";		
			$body = "Dear Parent | $fullname | Your Registration request has been rejected.";
			$senderEmail = "From: sidrachoudry56@gmail.com";
		...
		$delete_query1 = "DELETE FROM parents WHERE id='$user_id'";
	...
	<th>ID</th> <th>Fullname</th> <th>Email</th> <th>Gender</th>  <th>Child ID</th> <th>Action</th>
	</tr>";
	...
	if($result1_count >0)
	{
		while ($row = mysqli_fetch_array($query_result1))
		{
			...
			$query_data2 = "SELECT * FROM parents WHERE id = '$db_id'";
